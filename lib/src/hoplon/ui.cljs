(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.utils  :refer [clean dash name pad descend? current target FileList->vec with-ready part]]
...
(def set-with!      (formula #(o/set %     %2 (%3 %4)))) ;; add not-found support
...
(defn ->extrinsic [x]
  (cond (nil?     x) ""
        (number?  x) ""
        (keyword? x) (name x)
        (string?  x) x #_(doto x (prn :string))
...
(defn positionable [ctor]
  ...
  (fn [{:keys [m mh mv ml mr mt mb g gh gv] :as attrs} views]
    ...
    (let [gho (cell= (/ (or gh g 0) 2))
      ...
      (swap-elems! views #(do (set-prop-with! (out %) "paddingLeft"   ->len %2)
                              (set-prop-with! (out %) "paddingRight"  ->len %2)) gho)
      (swap-elems! views #(do (set-prop-with! (out %) "paddingTop"    ->len %2)
                              (set-prop-with! (out %) "paddingBottom" ->len %2)) gvo)
      (with-let [v (ctor attrs views)]
        (set-prop-with! (in v) "marginLeft"   ->len (cell= (- (or ml mh m 0) gho)))
        (set-prop-with! (in v) "marginRight"  ->len (cell= (- (or mr mh m 0) gho)))
        (set-prop-with! (in v) "marginTop"    ->len (cell= (- (or mt mv m 0) gvo)))
        (set-prop-with! (in v) "marginBottom" ->len (cell= (- (or mb mv m 0) gvo)))))))
...
(defn transform [ctor]
  ...
  (fn [{:keys [x xx xy xz xb xs] :as attrs} elems]
    ...
    (with-let [e (ctor attrs elems)]
      (bind-in! e [mid .-style .-transform]       x)
...
(defn fieldable [ctor]
  ...
  (fn [{:keys [src commit] :as attrs} elems]
    (with-let [e (ctor attrs elems)]
      (set-with! (in e) "value" identity src)
...
(hoplon.core/defelem path
  "construct a vector image suitable for data visualizations where the extent
   is the sum of the fill and the padding.  this is necessary because svg strokes
  ...
  [{:keys [e eh ev m mh mv ml mr mt mb src k kc] :as attrs} elems]
  (let [ml (or m mh ml 0)
        mr (or m mh mr 0)
        mt (or m mv mt 0)
        mb (or m mv mb 0)
        eh (or e eh)
        ev (or e ev)]
    (svg :view-box [0 0 (+ eh ml mr) (+ ev mt mb)] (dissoc attrs :e :eh :ev :m :mh :mv :ml :mr :mt :mb :src :k :kc)
      (s/g :transform (translate ml (+ ev mt))
