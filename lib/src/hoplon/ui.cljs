(ns hoplon.ui
  ...
  (:require
    ...
    [hoplon.ui.attrs  :refer [+ / - r ratio? calc? ->attr ->dec extrinsic explicit translate]]
  ...
  (fn [{:keys [e eh ev eh- eh+ scroll overflowv] :as attrs} views]
    ...
    (binding [*ex* (or eh e)
      ...
      (with-let [v (ctor attrs views)]
        ...
        (bind-in! v [out .-style .-width]     (cell= (extrinsic *ex*)))
        (bind-in! v [out .-style .-minWidth]  (cell= (extrinsic  eh-)))
        (bind-in! v [out .-style .-maxWidth]  (cell= (extrinsic  eh+)))
        (bind-in! v [out .-style .-height]    (cell= (extrinsic *ey*)))
        (bind-in! v [mid .-style .-width]     (cell= (explicit  *ex*)))
        (bind-in! v [mid .-style .-minWidth]  (cell= (explicit   eh-)))
        (bind-in! v [mid .-style .-maxWidth]  (cell= (explicit   eh+)))
        (bind-in! v [mid .-style .-height]    (cell= (explicit  *ey*))) ;; cond fixes firefox vertical centering
        (bind-in! v [mid .-style .-maxHeight] (cell= (explicit  *ey*))) ;; todo: review
        (bind-in! v [mid .-style .-overflowY] overflowv) ;; todo: find better solution
        (bind-in! v [in  .-style .-overflowY] (cell= (when (and scroll *ey*) :auto))))))) ;; default likely breaks 100% height where a sibling overflows
...
(defn fillable [ctor]
  ...
  (fn [{:keys [fc fo fv] :as attrs} views]
    ...
    (with-let [v (ctor attrs views)]
      ...
      (bind-in! v [mid .-style .-opacity]    (cell= (when (ratio? fo) (str (->dec fo)))))
